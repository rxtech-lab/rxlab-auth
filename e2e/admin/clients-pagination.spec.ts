...
test.describe("Clients Pagination", () => {
  ...
  test("should handle page beyond total pages with redirect", async ({
    ...
    // Create 1 client so totalPages = 1
    await createOAuthClient(page, {
      name: "Redirect Test Client",
      redirectUri: "http://localhost:3001/callback",
    });

    // Navigate to page 5 which is beyond total pages
    await page.goto("/admin/dashboard/clients?page=5&pageSize=20");

    // Should redirect to page=1 (last valid page)
    await expect(page.getByTestId("page-info")).toContainText("Page 1 of 1");
