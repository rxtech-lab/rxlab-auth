...
interface UseAdminPasskeyOptions {
  onSuccess?: () => void;
  onError?: (error: Error) => void;
}

export function useAdminPasskey(options: UseAdminPasskeyOptions = {}) {
  ...
  const registerPasskey = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        const { sessionId, ...registrationOptions } =
          optionsData as PublicKeyCredentialCreationOptionsJSON & { sessionId: string };
        ...
        const verifyResponse = await fetch("/api/admin/passkey/register/verify", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name, credential, sessionId }),
        });
    ...
    async () => {
      ...
      try {
        ...
        const optionsResponse = await fetch(
          "/api/admin/passkey/authenticate/options",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
          }
        ...
        const verifyResponse = await fetch(
          "/api/admin/passkey/authenticate/verify",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
