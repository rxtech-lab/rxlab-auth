...
function makeRequest(body: Record<string, unknown>): Request {
  return new Request(
    "https://auth.rxlab.app/api/oauth/passkey/register/options",
    {
      method: "POST",
...
describe("POST /api/oauth/passkey/register/options", () => {
  beforeEach(() => {
    ...
    generateRegistrationOptionsMock.mockResolvedValue({
      ...
      user: { id: "uid", name: "user@example.com", displayName: "user" },
      pubKeyCredParams: [],
      attestation: "none",
    });
  });
  ...
  test("happy path: returns options + sessionId; persists redirect_uri on challenge", async () => {
    ...
    expect(storedChallenge.type).toBe("native-registration");
    expect(storedChallenge.pendingEmail).toBe("new@example.com");
  });
